...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kAlloy_Custom_FontsUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kAlloy_Custom_FontsUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
